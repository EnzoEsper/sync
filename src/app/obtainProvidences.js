...
const obtainProvidences = async (cookie, { name, id, caseFiles }) => {
  const client = new GraphQLClient("http://localhost:4466");

  console.log("TRATANDO LOCALIDAD:", name);
  ...
  console.log("CBO:", cbo);
  for (let u = 0; u < caseFiles.length; u++) {
    console.log("...............................");
    console.log("TRATANDO EXPEDIENTE NUMERO:", caseFiles[u].number);
    console.log("...............................");

    let caseFileNumber = caseFiles[u].number;
    let caseFileID = caseFiles[u].id;
    let caseFileNumberOfProvidences = caseFiles[u].numberOfProvidences;
    let caseFileCourt = caseFiles[u].court;
    let caseFileLinkCurrentCoverDB = caseFiles[u].linkCurrentCover;
    let caseFileLinksCoversDB = caseFiles[u].linksCovers;
    ...
    let caseFileResponse = await caseFileRequest(cookie, num1, num2, cbo);
    ...
    if (caseFileNumberOfProvidences > 0) {
      ...
      for (let i = 0; i < linksTbodyCourts.length; i++) {
        ...
        linksTbodyCourts[i] =
          linksTbodyCourts[i] + " " + providencesLinks.length;
      }
      ...
      if (actualNumberOfProvidences > caseFileNumberOfProvidences) {
        ...
        console.log("El expediente no tiene nuevas providencias");
      ...
      if (caseFile.header.organismo !== caseFileCourt) {
        ...
        try {
          ...
          fetch("http://localhost:3035/api/webhook/trigger/updateCourt", {
            method: "post",
            body: JSON.stringify({
              organismo: caseFile.header.organismo.trim()
            }),
            headers: { "Content-Type": "application/json" }
          })
            .then(res => res.json())
